...
set.seed(123); sce_591146 <- scDblFinder(sce_591146)
set.seed(123); sce_614658 <- scDblFinder(sce_614658)
...
doublet_summary <- data.frame(
  ...
  singlets=c(sum(sce_591146$scDblFinder.class=="singlet"),sum(sce_614658$scDblFinder.class=="singlet")),
  predicted_doublets=c(sum(sce_591146$scDblFinder.class=="doublet"),sum(sce_614658$scDblFinder.class=="doublet"))
)
doublet_summary$doublet_percent <- round(100*doublet_summary$predicted_doublets/doublet_summary$total_cells,2)
print(doublet_summary)
write.csv(doublet_summary, "results/doublet_summary.csv", row.names=FALSE)

sample1_clean <- subset(luad_split[["591146"]], subset=doublet_class=="singlet")
sample2_clean <- subset(luad_split[["614658"]], subset=doublet_class=="singlet")
luad_clean <- merge(sample1_clean, y=sample2_clean, project="GSE164983_LUAD_clean")
