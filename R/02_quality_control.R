...
ggsave("figures/01_QC_before_filtering.png", qc_plot, width = 12, height = 5, dpi = 300)
...
ggsave("figures/02_QC_scatter_before_filtering.png", scatter_plot, width = 11, height = 5, dpi = 300)

# Dataset-informed QC thresholds selected after examining sample-specific
# feature-count and mitochondrial distributions.
luad$QC_status <- ifelse(
  luad$nFeature_RNA >= 300 &
    luad$nFeature_RNA <= 6000 &
    luad$percent.mt <= 20,
  "Pass",
  "Fail"
)

qc_retention <- luad@meta.data %>%
  group_by(sample_id) %>%
  summarise(
    before = n(),
    passed = sum(QC_status == "Pass"),
    removed = sum(QC_status == "Fail"),
    retention_percent = round(100 * passed / before, 2),
    .groups = "drop"
  )
...
ggsave("figures/03_QC_after_filtering.png", qc_after, width = 12, height = 5, dpi = 300)
